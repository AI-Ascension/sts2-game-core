...
impl Policy {
    ...
    fn parse(text: &str) -> Result<Self, String> {
        ...
        if root.get("policy_version").and_then(Value::as_integer) != Some(SUPPORTED_POLICY_VERSION)
        {
            return Err(format!(
                "policy_version must be {SUPPORTED_POLICY_VERSION}, found {:?}",
                root.get("policy_version").and_then(Value::as_integer)
            ));
        }
