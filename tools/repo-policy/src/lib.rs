...
/// Checks the target repository policy under `root`.
///
/// # Errors
///
...
pub fn check(root: &Path, strict: bool) -> Result<Outcome, String> {
    ...
    findings.extend(boundary::findings(root, &repository_files));
...
mod tests {
    ...
    #[test]
    fn current_policy_keeps_unclassified_warning_blocking() {
        let findings = [Finding::warning("RUST001", "Cargo.toml", "weak")];
        let advisory = BTreeSet::from([String::from("SIZE001")]);
        let result = outcome(1, true, 2, &advisory, &findings);
        assert_eq!(result.blocking_warnings, 1);
        assert_eq!(result.errors, 1);
        assert!(!result.passed(true));
    }
}
